...
type Application struct {
	cli.Application
	// Hub is the Gravity Hub address to execute the command against
	Hub *string
	// Token is the Gravity authentication token
	Token *string
	// BuildCmd is the extended "tele build" command
	BuildCmd BuildCmd
	// LoginCmd logs into specified Gravity Hub and cluster
	LoginCmd LoginCmd
	// LogoutCmd clears current login information
	LogoutCmd LogoutCmd
	// StatusCmd displays current login information
	StatusCmd StatusCmd
	// PushCmd uploads app installer to Gravity Hub
	PushCmd PushCmd
