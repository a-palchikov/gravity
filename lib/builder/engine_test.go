...
func (s *InstallerBuilderSuite) TestBuildInstallerWithIntermediateHops(c *check.C) {
	...
	// TODO(dima): validate resulting manifest once the intermediate version
...
func (s *CustomImageBuilderSuite) TearDownTest(c *check.C) {
	s.builderCommon.close()
	s.client.RemoveImage(planetImageRef.String())
	s.client.RemoveImage(planetAutogeneratedImageRef)
}
...
func mustLoc(v string) loc.Locator {
	return loc.MustParseLocator(v)
}
